...
def IH_IH(
    ...
    jsonify(
        ...
        IH,
        IH,
        save_to=f"{out_dir}/{model_name}/subspace_IH_K{K}_{method}.json",
    )
    plot(
        s_match,
        match_baseline,
        IH,
        IH,
        save_to=f"{out_dir}/{model_name}/Figs/subspace_IH_K{K}_{method}.png",
    )
...
def PTH_PTH(
    ...
    jsonify(
        ...
        PTH,
        PTH,
        save_to=f"{out_dir}/{model_name}/subspace_PTH_K{K}_{method}.json",
    )
    plot(
        s_match,
        match_baseline,
        PTH,
        PTH,
        save_to=f"{out_dir}/{model_name}/Figs/subspace_PTH_K{K}_{method}.png",
    )
...
def IH_PTH(
    ...
    jsonify(
        ...
        IH,
        PTH,
        save_to=f"{out_dir}/{model_name}/subspace_IH_PTH_K{K}_{method}.json",
    )
    plot(
        s_match,
        match_baseline,
        IH,
        PTH,
        save_to=f"{out_dir}/{model_name}/Figs/subspace_IH_PTH_K{K}_{method}.png",
    )


def randomize(model_name, K, rank, method, out_dir="out"):
    ...
    IH = [IH[i] for i in np.random.choice(range(len(IH)), size=K)]
    PTH = [PTH[i] for i in np.random.choice(range(len(PTH)), size=K)]

    # IH
    s_match, match_baseline = subspace_matching_IH(
        W_all, IH=IH, rank=rank, method=method
    )
    jsonify(
        s_match,
        match_baseline,
        IH,
        IH,
        save_to=f"{out_dir}/{model_name}/subspace_IH_IH_K{K}_{method}_random.json",
    )

    baseline = np.mean(match_baseline)
    plt.title(f"Baseline: {baseline:.3f}")
    plt.hist(s_match.flatten(), bins=30, edgecolor="white")
    plt.axvline(x=baseline, color="red", linestyle="dashed")
    plt.savefig(
        f"{out_dir}/{model_name}/Figs/subspace_IH_IH_K{K}_{method}_random.png",
        bbox_inches="tight",
    )
    plt.close()

    # PTH
    s_match, match_baseline = subspace_matching_PTH(
        W_all, PTH=PTH, rank=rank, method=method
    )
    jsonify(
        s_match,
        match_baseline,
        PTH,
        PTH,
        save_to=f"{out_dir}/{model_name}/subspace_PTH_PTH_K{K}_{method}_random.json",
    )
    baseline = np.mean(match_baseline)
    plt.title(f"Baseline: {baseline:.3f}")
    plt.hist(s_match.flatten(), bins=30, edgecolor="white")
    plt.axvline(x=baseline, color="red", linestyle="dashed")
    plt.savefig(
        f"{out_dir}/{model_name}/Figs/subspace_PTH_PTH_K{K}_{method}_random.png",
        bbox_inches="tight",
    )
    plt.close()

    # IH PTH
    s_match, match_baseline = subspace_matching_IH_PTH(
        W_all, IH=IH, PTH=PTH, rank=rank, method=method
    )
    jsonify(
        s_match,
        match_baseline,
        IH,
        PTH,
        save_to=f"{out_dir}/{model_name}/subspace_IH_PTH_K{K}_{method}_random.json",
    )
    baseline = np.mean(match_baseline)
    plt.title(f"Baseline: {baseline:.3f}")
    plt.hist(s_match.flatten(), bins=30, edgecolor="white")
    plt.axvline(x=baseline, color="red", linestyle="dashed")
    plt.savefig(
        f"{out_dir}/{model_name}/Figs/subspace_IH_PTH_K{K}_{method}_random.png",
        bbox_inches="tight",
    )
    plt.close()
...
def main(
    ...
    if random:
        randomize(model_name=model_name, K=K, rank=rank, method=method, out_dir=out_dir)
    else:
        W_all = torch.load(f"checkpoints/{model_name}/W_all.pt")
        IH = torch.load(f"checkpoints/{model_name}/IH.pt")[:K]
        PTH = torch.load(f"checkpoints/{model_name}/PTH.pt")[:K]

        IH_IH(model_name, W_all, K, rank, IH, method, out_dir)
        PTH_PTH(model_name, W_all, K, rank, PTH, method, out_dir)
        IH_PTH(model_name, W_all, K, rank, IH, PTH, method, out_dir)
