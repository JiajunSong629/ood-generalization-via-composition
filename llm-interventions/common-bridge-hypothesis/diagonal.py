...
def plot(scores_nml, IH, PTH, save_to):
    ...
    ax.set_title("IH-Shifting matching", weight="bold")

    ax = axs[1]
    ax.hist(scores_nml.flatten(), bins=30, edgecolor="white")
    ax.set_title("Histogram of IH-PTH matching Z-score", weight="bold")

    plt.savefig(save_to, bbox_inches="tight")
    plt.close()


def randomize_diagonal_distribution(model_name, K):
    ...
    jsonify(scores, IH, PTH, save_to=f"out/{model_name}/diagonal_random.json")
    plt.hist(scores.flatten(), bins=50, edgecolor="white")
    plt.title("Histogram of IH-PTH matching Z-score", weight="bold")
    plt.savefig(
        f"out/{model_name}/Figs/diagonal_matching_random_{K}.png", bbox_inches="tight"
    )
    plt.close()


def main(model_name, K=20, random=False):
    create_folder(f"out/{model_name}/Figs")

    if random:
        randomize_diagonal_distribution(model_name, K=K)
        return
